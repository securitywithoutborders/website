---
title: "Exodus: New Android Spyware Made in Italy"
...
After download, *Exodus One* would dynamically load and execute the primary stage 2 payload `mike.jar` using the Android API [DexClassLoader()](https://developer.android.com/reference/dalvik/system/DexClassLoader). `mike.jar` implements most of the data collection and exfiltration capabilities of this spyware.
...
Similarly to another Android spyware made in Italy, originally discovered by [Lukas Stefanko](https://twitter.com/LukasStefanko/status/928548460332290048) and later named [Skygofree](https://securelist.com/skygofree-following-in-the-footsteps-of-hackingteam/83603/) and analyzed in depth by Kaspersky Labs, Exodus also takes advantage of "protectedapps", a feature in Huawei phones that allows to configure power-saving options for running applications. By manipulating a SQLite database, Exodus is able to keep itself running even when the screen goes off and the application would otherwise be suspended to reduce battery consumption.
...
This Command & Control seems to have been active since at least April 2017 and was registered impersonating the legitimate service [AccuWeather](https://www.accuweather.com).
...
A search for this certificate fingerprint on the Internet scanning service [Censys](https://www.censys.io) returns 8 additional servers:
...
At the time of writing, a reverse image search for the favicon on [Shodan](https://www.shodan.io) using the query `http.favicon.hash:990643579` returned around 40 web servers which use the same favicon.
...
Their [publicly advertised products](https://gfoeurope.it/public/File/Presentazioni%20Perugia/eSurv%20Present.%20GFO%20-%20Giungla%20Digitale%20Perugia.pdf) include CCTV management systems, surveillance drones, face and license plate recognition systems.
...
Interestingly, we found other DNS records mostly from 2017 that follow a similar pattern and appear to contain [two-letters codes](http://www.comuni-italiani.it/province.html) for districts in Italy:
